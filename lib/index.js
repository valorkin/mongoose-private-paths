...
module.exports = function(schema, options) {
  ...
  function toSelectPaths(p){
    return _.flatten(to(p)).join(' ');
    function to(pathes, parent) {
      ...
      return _.map(pathes, function (isPrivate, path) {
        if (isPrivate === true || isPrivate === false) {
          if (parent && parent.length){
            return '-' + [parent.join('.'), path].join('.');
          }

          return '-' + path;
        }
        return to(isPrivate, path);
      });
    }
  }
  schema._privatePathes = toSelectPaths(private_paths);
};
